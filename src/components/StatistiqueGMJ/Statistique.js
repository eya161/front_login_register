...
export default function Statistique() {
  ...
  const getClientData = async () => {
    try {
      ...
      const data = await axios.get(
        `https://127.0.0.1:8000/api/users/?roles=["ROLE_CLIENT"]`,
        config
      );
  ...
  const getCommandeData = async () => {
    try {
      ...
      console.log(config);
      const data = await axios.get(
        "https://127.0.0.1:8000/api/commandes",
        config
      );
      console.log(data.data);
  ...
  const getFilteredData = async (e) => {
    e.preventDefault(); // ma ta3melich refresh  
    if (dateFin < dateDebut) {
        ...
        try {
          const result = await axios.get( `https://127.0.0.1:8000/getproduitbyperiode?datedebut=${dateDebut}&datefin=${dateFin}`, config);
          ...
          <div class="row flex">
            ...
            <div class="col-lg-1 mb-4" style={{ marginTop: "30px" }}>
              <Button
                ...
                type="submit"
                className="button"
                style={{ backgroundColor: "#004b88b6" }}
              >
                <FaSearch />
              </Button>
            </div>
          ...
          <div class="row text-center">
            <div class="col-xs-12 col-sm-6 col-xl-6 mb-3">
              <Table striped bordered hover>
                <thead style={{ background: "#c9def7be" }}>
                  <tr>
                    <th colSpan={3}>Clients/Produits</th>
                  </tr>
                  <tr>
            ...
            <div class="col-xs-12 col-sm-6 col-xl-6 mb-3">
              <Table striped bordered hover>
                <thead style={{ background: "#c9def7be" }}>
                  <tr>
                    <th colSpan={3}>Produits/Commandes</th>
                  </tr>
                  <tr>
                    <th>Référence</th>
                    <th>Quantité</th>
